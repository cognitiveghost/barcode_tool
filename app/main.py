...
# Windows. When GTK3 isn't on PATH - or another tool's older fontconfig.dll
# shadows it - fontconfig can't find its fonts.conf and prints "Cannot load
# default config file" at startup. Labels still render (this app's
# templates use a bundled @font-face TTF, not system font lookup), but
# pointing FONTCONFIG_PATH at a real fonts.conf when one is findable avoids
# the noise. Best-effort only: never overrides an already-set env var, and
