...
- **Spec coverage:** Mode 2.1's corridor/number/height/prefix/custom-text rules (Tasks 3, 5, 7) match design doc §5 exactly, including the hidden-warehouse-prefix rule (Task 4's `write_text=False` + Task 7's separate `barcode_data`/`visible_text` construction). Settings (Task 6) covers warehouses, shared folder, default printer per design §9; label size presets ship via `DEFAULT_SETTINGS` (Task 2). Printing (Task 8) and audit logging (Task 9) match design §§10-11, including "log at print time, not generation time." Modes 2.2-2.4 and enhancements are intentionally deferred to the roadmap-level section per the user's request for this session.
